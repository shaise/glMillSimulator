...
GuiItem guiItems[] = {
    ...
    {eGuiItemRotate,          0, 0, 630, 550, ' ', false, GUIITEM_TOGGLE_BUTTON},
    ...
    {eGuiItemPath,            0, 0, 670, 550, 'L', false, GUIITEM_TOGGLE_BUTTON},
    {eGuiItemAmbientOclusion, 0, 0, 710, 550, 'A', false, GUIITEM_TOGGLE_BUTTON},
};
...
void GuiDisplay::RenderItem(int itemId)
{
    ...
    else if (item->flags & GUIITEM_TOGGLED) {
...
void MillSim::GuiDisplay::HandleActionItem(GuiItem* guiItem)
{
    if (guiItem->actionKey >= 32) {
        if (guiItem->flags & GUIITEM_TOGGLE_BUTTON) {
            guiItem->flags ^= GUIITEM_TOGGLED;
        }
        bool isChecked = (guiItem->flags & GUIITEM_TOGGLED) != 0;
